...
<template>
    <section class="overflow-hidden md:py-[100vh] sm:py-[50vh] py-[25vh]">
        <div
            class="sm:h-screen flex items-center justify-center sm:flex-row flex-col md:gap-10 sm:gap-6 gap-2"
        >
            <div class="flex flex-col gap-[inherit]" ref="portfolio-imgs-backward">
                <img
                    v-for="(img, index) in oddScreenShotImgs"
                    :key="'oddScreenShot-' + index"
                    :src="img"
                    alt=""
                    class=""
                />
            ...
            <div class="flex flex-col gap-[inherit]" ref="portfolio-imgs-forward">
                <img
                    v-for="(img, index) in evenScreenShotImgs"
                    :key="'evenScreenShot-' + index"
                    :src="img"
                    alt=""
                    class=""
                />

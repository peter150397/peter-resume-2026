...
const data = ref<HomeServiceItem[]>([])
const getData = async () => {
    const res = await axios.get(`/HomeService`)
...
onMounted(async () => {
    ...
    document.fonts.ready.then(() => {
        if (
            !homeServiceItemsEle.value ||
            !homeServiceFrameEle.value ||
            !homeServiceItemsContainerEle.value
        )
            return
        ...
        const frameTl = gsap.timeline({
            scrollTrigger: {
                ...
                start: (self) => {
                    const selfHeight = self.trigger
                        ? self.trigger.getBoundingClientRect().height
                        : 0
        ...
        frameTl
            ...
            .to(
                homeServiceFrameEle.value,
                frameConfig(1, undefined, isDesktop ? itemsContainerWidth - frameWidth : 0),
            )
        ...
        <div class="container">
            ...
            <div class="relative z-1 max-w-225 mx-auto" ref="home-service-item-container">
                <div
                    ...
                    :class="
                        index % 2 === 1
                            ? 'lg:grid-cols-[1fr_400px] md:grid-cols-[1fr_300px] sm:grid-cols-[1fr_200px]'
                            : 'lg:grid-cols-[400px_1fr] md:grid-cols-[300px_1fr] sm:grid-cols-[200px_1fr]'
                    "
                ...
                >
                    ...
                    <div class="" :class="index % 2 === 1 ? 'sm:-order-1' : ''">
                        <h3
                            ...
                            :class="'home-service-item-content-' + index"
                        >
                            {{ item.title }}
                        </h3>
                        <p class="overflow-hidden" :class="'home-service-item-content-' + index">
                            {{ item.content }}
                        </p>
                    </div>
                </div>
                <div
                    class="absolute -z-1 lg:top-[calc(50vh-200px)] md:top-[calc(50vh-150px)] sm:top-[calc(50vh-100px)] top-(--navbar-height) sm:left-0 left-[calc(50%-120px)] lg:w-100 md:w-75 sm:w-50 w-60 aspect-square"

...
<template>
    <section class="h-screen relative z-1 overflow-hidden" id="portfolio-single-banner">
        ...
        >
            <SplitTextTitle
                trigger="#portfolio-single-banner"
                ele="h1"
                :title="props.title"
                class="text-center"
            />
            <SplitTextTitle
                trigger="#portfolio-single-banner"
                ele="h4"
                :title="props.banner.creation_time"
                class="text-center mb-6 mt-4"
            />

            <MainLink
                v-if="props.banner.link"
                :url="props.banner.link.url"
                :text="props.banner.link.text"
                :is-external="props.banner.link.is_external"
                class="mx-auto"
            />
        </div>
        <!-- Scroll Down Icon -->
        <div
            class="absolute bottom-6 left-1/2 -translate-x-1/2 mix-blend-difference flex items-center gap-2"
        >
